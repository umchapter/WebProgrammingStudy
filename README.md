...
> * 컨볼루션 연산 \
화소 영역 처리에서는 일정한 영역에 대하여 다음과 같은 합성곱 연산을 수행한다. $$ \displaystyle \sum_{m=(x-k)}^{x+k} \sum_{(n=y-k)}^{y+k}(\bold I[m,n] \times \bold M[m,n])$$ ( $k:$마스크의 크기, $ __I__:$컨볼루션의 대상이 되는 이미지의 영역, $ __M__:$마스크 행렬 )
### 1. 엠보싱(Embossing)
